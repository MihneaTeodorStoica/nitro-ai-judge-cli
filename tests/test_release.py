...
class ReleaseTests(unittest.TestCase):
    def test_3_0_3_versions_keep_protocol_compatibility(self) -> None:
        pyproject = Path(__file__).parents[1].joinpath("pyproject.toml").read_text()
        self.assertIn('version = "3.0.3"', pyproject)
        self.assertEqual(MANAGER_VERSION, "3.0.3")
        self.assertEqual(
            DEFAULT_MANAGER_IMAGE,
            ...
            with self.subTest(configured=configured, explicit=explicit):
                ...
                with (
                    ...
                    patch.object(
                        play,
                        "install_manager",
                        return_value={"manager_version": "3.0.3"},
                    ) as install,
