...
class ReleaseTests(unittest.TestCase):
    def test_3_0_4_versions_keep_protocol_compatibility(self) -> None:
        pyproject = Path(__file__).parents[1].joinpath("pyproject.toml").read_text()
        self.assertIn('version = "3.0.4"', pyproject)
        self.assertEqual(MANAGER_VERSION, "3.0.4")
        self.assertEqual(
            DEFAULT_MANAGER_IMAGE,
            "ghcr.io/mihneateodorstoica/naij-play-manager:3.0.4",
        )
